def trio_adding_upto_2020(array)
  ...
  while (left_pointer != array.length - 2) && (middle_pointer != array.length - 1) && (right_pointer != array.length)
    (middle_pointer...array.length).each do |middle|
      (right_pointer...array.length).each do |right|
        ...
        return [array[left_pointer], array[middle], array[right]] if sum == 2020
      end
    end
